...
public interface FriendRequestRepository extends JpaRepository<FriendRequest, Long> {
    ...
    @EntityGraph(attributePaths = {"sender", "receiver"})
    List<FriendRequest> findByReceiverIdAndStatusOrderByCreatedAtDesc(Integer receiverId, FriendRequestStatus status);

    @EntityGraph(attributePaths = {"sender", "receiver"})
    List<FriendRequest> findBySenderIdAndStatusOrderByCreatedAtDesc(Integer senderId, FriendRequestStatus status);
    ...
    """)
    void deleteRequestsBetweenUsers(@Param("user1") User user1, @Param("user2") User user2);
}
