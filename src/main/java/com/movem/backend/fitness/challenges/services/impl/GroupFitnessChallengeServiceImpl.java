...
import com.movem.backend.fitness.challenges.mappers.GroupFitnessChallengeMapper;
...
import com.movem.backend.fitness.challenges.services.GroupFitnessChallengeService;
...
public class GroupFitnessChallengeServiceImpl implements GroupFitnessChallengeService {
    ...
    @Override
    public GroupFitnessChallengeResponse createChallenge(Integer clubId, CreateGroupFitnessChallengeRequest request) {
        ...
        FitnessChallengeStatus challengeStatus = determineStatus(values.startAt(), values.endAt(), now);
        ...
        Activity activity = activityService.createActivity(activitySource, currentUser, ActivityType.FITNESS);
        ...
        featureEventTrackingService.handle(fitnessChallengeEventFactory.created(saved, currentUser));
    ...
    @Override
    @Transactional
    public GroupFitnessChallengeResponse getChallenge(Integer challengeId) {
        GroupFitnessChallenge challenge = groupFitnessChallengeRepository.findById(challengeId).orElseThrow(() -> new ResourceNotFoundException("Group fitness challenge not found."));
    ...
    getClubChallenges(Integer clubId) {
        FitnessClub club = getClub(clubId);
        return groupFitnessChallengeRepository
                .findByFitnessClubOrderByCreatedAtDesc(club)
                .stream()
                .map(groupFitnessChallengeMapper::toResponse)
                .toList();
    }
    ...
    @Override
    public GroupFitnessChallengeResponse updateChallenge(Integer challengeId, UpdateGroupFitnessChallengeRequest request) {
        User currentUser = currentUserService.getCurrentUser();

        GroupFitnessChallenge challenge = groupFitnessChallengeRepository.findById(challengeId).orElseThrow(() -> new ResourceNotFoundException("Group fitness challenge not found."));
        FitnessClubMember membership = getMembership(challenge.getFitnessClub(), currentUser);
        requireChallengeManagementPermission(membership, challenge);
        ...
        FitnessChallengeStatus status = determineStatus(request.getStartAt(), request.getEndAt(), LocalDateTime.now());
        ...
        if (activity != null) {
            ...
            activity.setStartActivity(request.getStartAt());
            activity.setDeadline(request.getEndAt());
            activity.setStatus(mapToActivityStatus(status));
        ...
        featureEventTrackingService.handle(fitnessChallengeEventFactory.updated(saved, currentUser));
    ...
    public void deleteChallenge(Integer challengeId) {
        ...
        challenge.setStatus(FitnessChallengeStatus.CANCELLED);
        challenge.setUpdatedAt(LocalDateTime.now());

        groupFitnessChallengeRepository.save(challenge);

        featureEventTrackingService.handle(fitnessChallengeEventFactory.cancelled(challenge, currentUser));
    }

    @Override
    public GroupFitnessChallengeResponse createChallengeFromCatalog(Integer clubId, Integer catalogId, CreateGroupFitnessChallengeFromCatalogRequest request) {

        User currentUser = currentUserService.getCurrentUser();

        FitnessClub club = fitnessClubRepository.findById(clubId).orElseThrow(() -> new ResourceNotFoundException("Fitness club not found."));

        FitnessClubMember membership = fitnessClubMemberRepository.findByFitnessClubAndUser(club, currentUser)
                        .orElseThrow(() -> new UnauthorizedActionException("You are not a member of this fitness club."));

        if (membership.getRole() != FitnessClubRole.OWNER && membership.getRole() != FitnessClubRole.ADMIN) {
            throw new UnauthorizedActionException("You do not have permission to create a fitness challenge.");
        }
        GroupChallengeCatalog catalog = groupChallengeCatalogRepository.findById(catalogId).orElseThrow(() -> new ResourceNotFoundException("Group challenge catalog not found."));

        validateDates(request.getStartAt(), request.getEndAt());

        LocalDateTime now = LocalDateTime.now();

        FitnessChallengeStatus status = determineStatus(request.getStartAt(), request.getEndAt(), now);
        FitnessChallengeCreateSource activitySource = new FitnessChallengeCreateSource();

        activitySource.setActivityName(catalog.getName());
        activitySource.setDescription(catalog.getDescription());
        activitySource.setStartActivity(request.getStartAt());
        activitySource.setDeadline(request.getEndAt());
        activitySource.setParentActivityId(null);
        ...
        challenge.setActivity(activity);
        challenge.setFitnessClub(club);
        challenge.setCatalog(catalog);
        challenge.setChallengeSource(ChallengeSource.RECOMMENDED);
        challenge.setName(catalog.getName());
        challenge.setWorkoutType(catalog.getWorkoutType());
        challenge.setTargetValue(catalog.getTargetValue());
        challenge.setTargetUnit(catalog.getTargetUnit());
        challenge.setDescription(catalog.getDescription());
        challenge.setCreatedBy(currentUser);
        challenge.setStartAt(request.getStartAt());
        challenge.setEndAt(request.getEndAt());
        challenge.setStatus(status);
    ...
    private FitnessClubMember getMembership(FitnessClub club, User user) {
        return fitnessClubMemberRepository.findByFitnessClubAndUser(club, user)
                .orElseThrow(() -> new IllegalArgumentException("You are not a member of this fitness club."));
    }

    @Override
    @Transactional
    public SocialChallengeResponse getSocialChallenge(Integer challengeId) {
        User currentUser = currentUserService.getCurrentUser();

        GroupFitnessChallenge challenge = groupFitnessChallengeRepository.findById(challengeId).orElseThrow(() -> new ResourceNotFoundException("Group fitness challenge not found."));

        boolean isOwner = challenge.getCreatedBy().getId().equals(currentUser.getId());

        boolean isParticipant = participantRepository.findByChallengeAndUser(challenge, currentUser)
                        .isPresent();

        if (!isOwner && !isParticipant) {
            throw new UnauthorizedActionException("You do not have access to this challenge.");
        }

        long participantCount = participantRepository.countByChallenge(challenge);
        long completedParticipants = participantRepository.findByChallenge(challenge).stream().filter(participant -> participant.getStatus() == FitnessChallengeParticipantStatus.COMPLETED).count();

        FitnessChallengeParticipant myParticipant = participantRepository.findByChallengeAndUser(challenge, currentUser).orElse(null);
        BigDecimal myProgress = myParticipant != null ? calculateProgress(myParticipant) : BigDecimal.ZERO;

        boolean myCompleted = myParticipant != null && myParticipant.getStatus() == FitnessChallengeParticipantStatus.COMPLETED;
    ...
    private void requireChallengeCreationPermission(FitnessClubMember membership) {
        if (membership.getRole() != FitnessClubRole.OWNER && membership.getRole() != FitnessClubRole.ADMIN) {
            throw new IllegalArgumentException("You do not have permission to create fitness challenges in this club.");
        }
    }


    private void requireChallengeManagementPermission(FitnessClubMember membership, GroupFitnessChallenge challenge) {
        boolean isOwnerOrAdmin = membership.getRole() == FitnessClubRole.OWNER || membership.getRole() == FitnessClubRole.ADMIN;
        boolean isCreator = challenge.getCreatedBy().getId().equals(membership.getUser().getId());
        if (!isOwnerOrAdmin && !isCreator) {
            throw new IllegalArgumentException("You do not have permission to manage this challenge.");
        }
    }
    ...
    private FitnessChallengeStatus determineStatus(LocalDateTime startAt, LocalDateTime endAt, LocalDateTime now) {
    ...
    private ActivityStatus mapToActivityStatus(FitnessChallengeStatus status) {
    ...
    private ChallengeValues resolveChallengeValues(CreateGroupFitnessChallengeRequest request, GroupChallengeCatalog catalog) {
        if (catalog != null) {
    ...
    private record ChallengeValues(String name, WorkoutType workoutType, BigDecimal targetValue, ChallengeTargetUnit targetUnit, String description, LocalDateTime startAt, LocalDateTime endAt) { }

    private BigDecimal calculateProgress(FitnessChallengeParticipant participant) {
        List<FitnessWorkoutSession> sessions = fitnessWorkoutSessionRepository.findByGroupChallengeParticipant(participant);
        return sessions.stream().filter(session -> session.getStatus() == FitnessWorkoutStatus.COMPLETED)
