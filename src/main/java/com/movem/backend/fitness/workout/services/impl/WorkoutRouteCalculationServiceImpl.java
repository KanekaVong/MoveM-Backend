...
@Service
public class WorkoutRouteCalculationServiceImpl implements WorkoutRouteCalculationService {
    private static final double EARTH_RADIUS_KM = 6371.0;

    @Override
    public BigDecimal calculateDistance(List<FitnessWorkoutRoutePoint> points) {
        ...
        for (int i = 1; i < points.size(); i++) {
            ...
            FitnessWorkoutRoutePoint previous = points.get(i - 1);
            FitnessWorkoutRoutePoint current = points.get(i);

            double latitude1 = Math.toRadians(previous.getLatitude().doubleValue());
            double longitude1 = Math.toRadians(previous.getLongitude().doubleValue());
            double latitude2 = Math.toRadians(current.getLatitude().doubleValue());
            double longitude2 = Math.toRadians(current.getLongitude().doubleValue());
        ...
        return BigDecimal.valueOf(totalDistance).setScale(3, RoundingMode.HALF_UP);
    }
    ...
    @Override
    public BigDecimal calculatePace(BigDecimal distanceKm, Integer durationSeconds) {
