...
public class TaskController {
    ...
    public ResponseEntity<TaskResponse> getTask(@PathVariable String activityId) {
        return ResponseEntity.ok(taskService.getTask(activityId));
    }
    ...
    public ResponseEntity<TaskResponse> updateTask(@PathVariable String activityId, @RequestBody @Valid UpdateTaskRequest request) {
        return ResponseEntity.ok(taskService.updateTask(activityId, request));
    }

    @DeleteMapping("/{activityId}")
    public ResponseEntity<Void> deleteTask(@PathVariable String activityId) {
        taskService.deleteTask(activityId);
        return ResponseEntity.noContent().build();
    }

    @PutMapping("/{activityId}/restore")
    public ResponseEntity<TaskResponse> restoreTask(@PathVariable String activityId) {
        return ResponseEntity.ok(taskService.restoreTask(activityId));
    }

    @PatchMapping("/{activityId}/complete")
    public ResponseEntity<TaskResponse> completeTask(@PathVariable String activityId) {
        return ResponseEntity.ok(taskService.completeTask(activityId));
    }
}
